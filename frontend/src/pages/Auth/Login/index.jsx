...
export default function Login() {
  ...
  const handleLogin = (e) => {
    ...
    if (username === 'admin' && password === 'admin') {
      ...
      // Aguardar 3 segundos antes de navegar
      setTimeout(() => {
        navigate('/dashboard');
      }, 3000);
    } else {
      // Login de Usuário Comum (Simulado) ou Falha
      // Para simular login de user comum com loading também:
      if (username !== 'admin') {
         setIsLoading(true);
         setTimeout(() => {
           navigate('/user');
        ...
        <div className={styles.rightSide}>
          <div className={styles.formContent}>
            ...
            <div className={styles.footer}>
              Não tem uma conta? <Link to="/cadastrar">Cadastre-se agora</Link>
            </div>
          </div>
        </div>
      </div>
